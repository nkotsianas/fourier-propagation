function [out] = Fraun_Rect_Example_Length_Tests (Lx,Ly)
...
out = I2(Ny/2+1,Mx/2+1)/I2A(Ny/2+1,Mx/2+1);
...
% legend('I2','I2A');
% % axis([x(1) x(length(x)) 0 max(I2(Ny/2+1,:))]);
% % axis([x(1) x(length(x)) 0 1e-5]);
% xlabel('x (m)'); ylabel('Irradiance');
% % figure(6)
% % plot(x,I2A(Ny/2+1,:));
% % % axis([x(1) x(length(x)) 0 max(I2A(Ny/2+1,:))]);
% % % axis([x(1) x(length(x)) 0 1e-5]);
% % xlabel('x (m)'); ylabel('Irradiance');
